 ...
 * com.liferay.portal.kernel.module.framework.ModuleServiceLifecycle; import
 ...
 * com.liferay.portal.kernel.scheduler.StorageType; import
 * com.liferay.portal.kernel.scheduler.StorageTypeAware; import
 ...
 * 
 * @Component( immediate = true, property = {"cron.expression=0/1 0 0 * * ?"},
 * service = SecondScheduler.class ) public class SecondScheduler extends
 * BaseMessageListener {
 * 
 *//**
 ...
 * SchedulerException {
 * 
 * // extract the cron expression from the properties String cronExpression =
 * GetterUtil.getString(properties.get("cron.expression"),
 * _DEFAULT_CRON_EXPRESSION);
 * 
 * // create a new trigger definition for the job. String listenerClass =
 * getClass().getName(); Trigger jobTrigger =
 * _triggerFactory.createTrigger(listenerClass, listenerClass, new Date(), null,
 * cronExpression);
 * 
 * // wrap the current scheduler entry in our new wrapper. // use the persisted
 * storaget type and set the wrapper back to the class field.
 * _schedulerEntryImpl = new SchedulerEntryImpl(getClass().getName(),
 * jobTrigger); _schedulerEntryImpl = new
 ...
 * 
 * // if we were initialized (i.e. if this is called due to CA modification) if
 * (_initialized) { // first deactivate the current job before we schedule.
 * deactivate(); }
 * 
 * // register the scheduled task _schedulerEngineHelper.register(this,
 * _schedulerEntryImpl, DestinationNames.SCHEDULER_DISPATCH);
 * 
 * // set the initialized flag. _initialized = true; }
 * 
 *//**
	 * deactivate: Called when OSGi is deactivating the component.
	 */
/*
 * @Deactivate protected void deactivate() { // if we previously were
 * initialized if (_initialized) { // unschedule the job so it is cleaned up try
 * { _schedulerEngineHelper.unschedule(_schedulerEntryImpl, getStorageType()); }
 * catch (SchedulerException se) { if (_log.isWarnEnabled()) {
 * _log.warn("Unable to unschedule trigger", se); } }
 * 
 * // unregister this listener _schedulerEngineHelper.unregister(this); }
 * 
 * // clear the initialized flag _initialized = false; }
 * 
 *//**
 ...
 *//**
	 ...
	 *//*
		 ...
		 * protected void setModuleServiceLifecycle(ModuleServiceLifecycle
		 * moduleServiceLifecycle) { }
		 * 
		 * @Reference(unbind = "-") protected void setTriggerFactory(TriggerFactory
		 * triggerFactory) { _triggerFactory = triggerFactory; }
		 * 
		 * @Reference(unbind = "-") protected void
		 * setSchedulerEngineHelper(SchedulerEngineHelper schedulerEngineHelper) {
		 * _schedulerEngineHelper = schedulerEngineHelper; }
		 * 
		 * // the default cron expression is to run daily at midnight private static
		 * final String _DEFAULT_CRON_EXPRESSION = "0 0 0 * * ?";
		 * 
		 * private static final Log _log = LogFactoryUtil.getLog(SecondScheduler.class);
		 * 
		 * private volatile boolean _initialized; private TriggerFactory
		 * _triggerFactory; private SchedulerEngineHelper _schedulerEngineHelper;
